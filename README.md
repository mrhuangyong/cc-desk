...
- **对 `~/.claude` 真实配置的读写** — 设置页直接读写 `~/.claude` 的 settings / mcpServers / plugins / skills / commands / hooks，不是 mock。
...
| **MCP / 技能 / 插件 / 命令 / Hooks** | 直读 `~/.claude` 真实配置 |
| **记忆** | 编辑 CLAUDE.md |

应用数据全部收敛到 `~/.cc-desk/`（settings / projects / config 三文件），与 `~/.claude/` 隔离。
...
│   ├── claude-config.ts           # ~/.claude 配置读写（深合并）
...
主进程 (Node.js)
    ...
    ├── FileService → fs
    └── SettingsStore / claude-config → ~/.cc-desk + ~/.claude
```
...
- **测试隔离** — 涉及落盘的主进程测试用临时 HOME + `vi.resetModules()`，绝不写真机 `~/.claude`。
