  ...
  return {
    ...
    route(env) {
      ...
      if (env.deviceId.startsWith('share:')) {
        ...
        let delivered = false
        if (deskId) {
          const set = conns.get(deskId)
          if (set && deskId !== env.deviceId) {
      ...
      if (tokenVirtual && tokenVirtual !== env.deviceId) {
        const tokenSet = conns.get(tokenVirtual)
        if (tokenSet) { for (const send of tokenSet) { send(env); delivered = true } }
      }
      return delivered
        ? { ok: true, delivered: true }
        : { ok: true, delivered: false, reason: 'peer_offline' }
