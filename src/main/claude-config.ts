...
// ---- 插件/技能/命令/hooks 扫描结果缓存 ----
// 这些扫描（getPlugins/getSkills/getCommands/getHooksFull）每次都做大量 FS I/O
// （读 installed_plugins.json + settings.json + 每个插件 manifest + 每个 SKILL.md/命令 md），
// 而 getSkills 在每次 send()（每条用户消息）的热路径上调用。所有读写都在主进程内，
// 故用 bust-on-write 缓存：写操作完成后置空对应缓存，下次读重新扫描。
// 失效粒度：改 installed_plugins/enabledPlugins → 全失效（影响所有扫描）；
// 仅改 disabledSkills 或 SKILL.md → 只失效 skills；仅改命令 md → 只失效 commands；
// 仅改 hooks → 只失效 hooks。
let pluginsCache: ClaudePlugin[] | null = null
let skillsCache: ClaudeSkill[] | null = null
let commandsCache: ClaudeCommand[] | null = null
let hooksCache: HooksFull | null = null

// 改了 installed_plugins.json 或 enabledPlugins：所有扫描都依赖这两者，全失效。
// 也供 marketplace-manager.removeMarketplace（跨模块改这两者）调用。
export function invalidateAllPluginScan(): void {
  pluginsCache = null
  skillsCache = null
  commandsCache = null
  hooksCache = null
}
function invalidateSkills(): void { skillsCache = null }
function invalidateCommands(): void { commandsCache = null }
function invalidateHooks(): void { hooksCache = null }
...
export async function saveSkillFile(id: string, content: string): Promise<void> {
  invalidateSkills() // 先失效，确保拿到当前正确的 path（getSkills 重扫）
  const skill = (await getSkills()).find(s => s.id === id)
  if (!skill || !skill.path) return
  await writeFile(skill.path, content, 'utf-8')
  invalidateSkills() // frontmatter 的 name/desc 可能随内容变化，再次失效
}
...
export async function saveCommandFile(name: string, content: string): Promise<void> {
  ...
  const filePath = join(CLAUDE_DIR, 'commands', `${cleanName}.md`)
  await writeFile(filePath, content, 'utf-8')
  invalidateCommands() // frontmatter description 可能变化
}
